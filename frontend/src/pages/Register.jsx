...
export const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Compte créé avec succès ! Connectez-vous.', { id: toastId });
      navigate('/login');
    } catch (err) {
      toast.error(err.message || 'Échec de l\'inscription', { id: toastId });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass border border-slate-800 p-8 rounded-3xl shadow-2xl relative">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Rejoignez-nous pour atteindre vos objectifs de santé</p>
        </div>
        ...
        <div className="text-center mt-6 text-sm text-slate-400">
          ...
          >
            <ArrowLeft size={14} /> Connectez-vous
          </Link>
        </div>
      </div>
    </div>
